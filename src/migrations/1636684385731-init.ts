...
export class init1636684385731 implements MigrationInterface {
    name = 'init1636684385731'
...
클라우드
 ...
 AZURE
  etc', PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`event\` (\`id\` int NOT NULL AUTO_INCREMENT, \`provider_key\` varchar(255) NULL, \`provider_code\` varchar(255) NULL, \`category\` varchar(255) NULL, \`image_url\` varchar(255) NULL, \`issued_at\` datetime NULL, \`status\` enum ('SENDING', 'COMPLETE') NOT NULL DEFAULT 'SENDING', \`user_id\` int NULL, \`user_mapping_id\` int NULL, \`message_id\` int NULL, \`event_type_id\` int NULL COMMENT 'Db generic id', \`provider_id\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
